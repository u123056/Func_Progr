...
rbgToCmyk :: RGB -> CMYK
rbgToCmyk (RGB red green blue) = let r = fromIntegral red / 255.0
...
geomProgression b q n|n<0 = error "negative num"  
geomProgression b q 0 = b
geomProgression b q n = (geomProgression b q (n-1)) * q 
...
coprime a b = if mod (max a1 b1) (min a1 b1) > 1 
				 then coprime (min a1 b1) (mod (max a1 b1) (min a1 b1)) 
				 else (if mod (max a1 b1) (min a1 b1) == 1 then True else False)
                 where a1 = abs a
                       b1 = abs b 
